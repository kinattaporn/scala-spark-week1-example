 ...
 */
 class ListsSuite {
   ...
   * when writing tests.
   */
  @Test def `details why one plus one is two (0pts)`: Unit = {
    Assert.assertEquals(2, 1 + 1) // Correct
  }
   ...
   */
   @Test def `intNotZero throws an exception if its argument is 0`: Unit = {
     try {
       ...
       case e: IllegalArgumentException => (println("janjan_IllegalArgumentException"))
     }
   }
  @Test def `intNotZero NOT throws an exception if its argument is 1`: Unit = {
    try {
      intNotZero(1)
      Assert.fail("No exception has been thrown")
    } catch {
  ...
  @Test def `intNotZero throws OTHER exception if its argument is 5/0`: Unit = {
    try {
      intNotZero(5/0)
      Assert.fail("No exception has been thrown")
    } catch {
